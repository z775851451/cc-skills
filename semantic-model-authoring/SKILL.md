...
description: "管理 Power BI 语义模型，包括表、列、度量值、关系、DAX 编写与调优、Import/DirectQuery/Direct Lake 设计、数据源、连接、权限、刷新和部署。在对语义模型执行操作前加载此技能，除非属于发布 PBIP 项目(由 powerbi-report-management 负责)。自然语言数据问题请使用 fabriciq；视觉对象请使用 powerbi-report-authoring。"
---
